...
export async function generateStaticParams() {
  ...
  const guideRoutes = [...allRoutes].filter(([route]) => route.startsWith("policies/"));
...
export default async function GuidePage({ params }: PageProps<{ slugs: string[] }>) {
  ...
  console.log("policies", ...slugs)
  const { Content, metadata, toc, ...rest } = await router.findOne("policies", ...slugs);
